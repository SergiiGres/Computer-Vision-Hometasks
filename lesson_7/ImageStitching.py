# Image Stitching
# In this notebook we are going to demonstrate how feature detection and feature descriptors can help us to perform
# image stitching (in this case, to compose a panoramic view from 2 images). Let's start with the usual imports.
...
# We will be working with 2 images that capture the same scene but with different camera position and different time.
# The objective is to transfrom the source image to align with the target image. In other words, we want to find the
# homography that thrasnforms the source view into the target view and use this homography to stitch the two images
# together.
...
# Feature Detection
# In order to find the abovementioned homography, we need to find the pixel correspondences.
# To do so, we will be using the well-known SIFT features (you can use any detector + descriptor you want).
...
# We see there is a lot of detected features and many of them are not relevant to our task (e.g. the waves on the
# river surface).

# Feature Matching Given the two sets of source and target features, we need to find the correspondences, i.e.,
# for each feature on the source image we need to find the best matching feature on the target image. In this
# notebook, we will be using the brute-force matcher with cross-check.
...
# As you can see, the cross-check already took care and eliminated many of the irrelevant features (both on source
# and target side).
...
# Given the correspondeces (i.e. the set of source points and the set of target points) we are going to find the best
# fitting homography transform. We will also switch the RANSAC on since this is a powerful tool to eliminate outliers.
...
# Image Stitching Using the estimated homography, we are going to transform the source image to the target viewport
# and we will stitch the two images together.
