# Integral Image
#
# Integral image is an extremely useful concept when we are facing the problem of summing up (
# rectangular) image regions (in pixel or in feature domain). In this notebook, we are going to show how to copmpute
# the integral image. In addition, we are going to measure the time for building a matrix holding the regional sums
# and compare the classical approach with the integral image approach.
...
# crucial step. For this example, we are now going to huild the sum matrix using a 15x15 sliding window. For the sake
# of simplicity, we won't bother with the border areas.
...
# Integral Image As we see above, building the sum matrix using the sliding window approach is quite slow. In fact,
# it is prohibitevly slow for real-time applications. Fortunately, we can make use of the integral image. Let's first
# build the integral image and then use it to build the matrix of regional sums.
...
# By using the integral image the computation time has decreased 4 times! Not bad, but can we do better? We know that
# python is not extremely efficient when it comes to loops. Therefore, can we get (somehow) rid of them?
#
# The asnwer is that we can. We can obtain the same results but first adjusting the regional matrices (A. B, C,
# D) by rolling their axes. Afterwards, the calculation of the matrix of regional sums is just a summation of four
# matrices.
