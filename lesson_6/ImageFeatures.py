...
# Harris corner detector evaluates each image pixel for the so called "cornerness", i.e.,
# it measures how much the neighbourhood of each pixel changes in different directions.
# Pixels, whose neighborhood changes both vertically and horizontally are considered corners (or corner candidates).
src = gray.astype(np.float32)
dst = cv2.cornerHarris(src, blockSize=2, ksize=11, k=0.04)
plt.imshow(dst)
plt.show()

# Remember the cornerness equation:
# for edge pixel one of the eigenvalues (of the Harris matrix M) is very large and the other very small.
# This yields negative (with relatively large absolute value) cornerness index.
...
plt.subplot(122), plt.imshow(dst)
rows, cols = dst.shape
th = 0.1 * np.max(dst)  # Keep going down to 0.01 -> outliers
result = np.copy(img)
